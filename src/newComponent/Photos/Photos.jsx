  ...
  const [user, setUser] = useContext(AuthContext);
   ...
   const [hover, setHover] = useState(false);
    ...
    modals.openConfirmModal({
      ...
      children: (
        <Text size="sm">
          Are you sure you want to delete Photo? This action is will delete photo and will be
          no longer available.
        </Text>
      ),
       ...
       if(user && user.photos) {
          const picture = user.photos.map((pho, ind) => {
            return {
               key: ind,
               id: pho.id,
              src: `${import.meta.env.VITE_BASE_URL}/photos/${pho.fileName}`,
              width: 800,
              height: 600,
            }
          })
...
const handlePhotoDelete = (photoId) => {
  ...
  return (
    <div className=" w-[90%] lg:w-[100%] mx-auto">
      ...
      <div className="w-full flex flex-row-reverse gap-2 items-center justify-start my-4 ">
         {!(photo.length >= 10) && (
           <Button size="lg" radius={'md'} variant="filled" style={{backgroundColor: 'var(--secondary)'}} rightIcon={<BsFillPlusCircleFill size={20} color="white" />} onClick={(e) => handleProfileClick(e)}>Add photo</Button>
         )}

            <Badge size="xl" radius={5} style={{backgroundColor: 'white', color: 'black'}}>
              {`${photo.length}/10`}
            </Badge>
  ...
  <>
      ...
      <div className="w-full h-full flex flex-col gap-2  md:flex-row flex-wrap">
         {photo.map((pho, index) => {
           return <SinglePhoto id={pho.id} openDeleteModal={openDeleteModal} key={index} index={index} setIndex={setIndex}  src={pho.src} />
         })}
      </div>
  </>
      ) : (
