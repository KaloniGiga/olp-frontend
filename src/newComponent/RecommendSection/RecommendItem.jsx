...
function RecommendItem(props) {
  ...
  const sendConnectionRequest = () => {
    console.log(user[0].id, props.recommend.id);
     dispatch(
      createConnectionRequestThunk(props.recommend.id)
     )
  }
  ...
  return (
    ...
    <div className=' rounded-lg bg-white overflow-hidden shadow-sm'>
       <div className='w-full h-[250px] overflow-hidden'>
        <img className='w-full h-full object-cover object-center' src={profileAvatar} alt="" />
       </div>

       <div className='flex flex-col px-2 py-2'>
          <h1 className='text-center text-[1.5rem] font-[500] my-2'>{props.recommend.username}</h1>
          <div className='w-full flex justify-between px-2'>
            ...
            <button className='px-2 py-2 w-full rounded-xl mr-1 text-md text-white bg-[#E61A52]' onClick={() => navigate(`/home/profile/${recommend.id}`)}>View Profile</button>
            <button className='px-2 py-2 w-full rounded-xl text-md border-[2px] border-[rgba(0, 0, 0, 0.4)]' onClick={() => sendConnectionRequest()}>Connect</button>
            </div>
      </div>
    </div>
    // </Link>
