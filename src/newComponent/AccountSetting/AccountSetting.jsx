...
function AccountSetting() {
  ...
  const dispatch = useDispatch();
   ...
   const openDeleteModal = () =>
    modals.openConfirmModal({
      ...
      children: (
        <Text size="sm">
          Are you sure you want to delete Account? This action is destructive and will permanently delete account  and
          all related data.
        </Text>
      ),
  ...
  const handlelogout = () => {
    axios.post(`${import.meta.env.VITE_BASE_URL}/authentication/log-out-after-delete`, {}, {
  ...
  return (
       ...
       <div className='flex border-b-2 py-4 px-4'>
          <span className='mr-4'><AiFillAccountBook color="var(--secondary)" size={30} /></span>
          <Title order={3}>Account Settings</Title>
       </div>

      <div className='flex flex-col mb-4 ml-4 px-2 py-2'>
          <Text size={'xl'} my={'sm'} fw='600'>Delete Account</Text>
          <div className='w-full px-2 '>
            <Button style={{ backgroundColor: 'var(--secondary)'}} size={largeDesktop ? 'xl' : (mediumDesktop ? 'md' : 'lg')} leftIcon={<AiOutlineDelete size={20} />} onClick={openDeleteModal} >Delete</Button>
          </div> 
      </div>
