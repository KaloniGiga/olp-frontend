...
import { ActionIcon, Badge, Button, Card, Chip, Group, Image, Stack, Text, Title, Tooltip } from '@mantine/core'
  ...
  return (
     <Card withBorder radius={'md'}>
        ...
        <Card.Section>
            ...
            <Image width={120} height={120} mx={'auto'} my="md" radius={100} src={props.recommend.avatarId ? `${import.meta.env.VITE_BASE_URL}/user-avatar/${props.recommend.avatarId}` : profileAvatar} />
        </Card.Section>
        <Stack justify='center' spacing={0} align='center'>
        <Title order={3}  transform='capitalize' >
        {props.recommend.fullname}
       </Title>
        <Text color='dimmed' align='center' fw={600} size={'sm'} transform='capitalize'>
      ...
      <Group position='center' my="sm">
        <Tooltip label="Age" withArrow>
         <Badge>
           {new AgeFromDate(new Date( props.recommend.year, monthToNum[props.recommend.month], props.recommend.day)).age}
         </Badge>
        </Tooltip>

        <Tooltip label="Religion" withArrow>
         <Badge >
            {props.recommend.religion}
         </Badge>
         </Tooltip>
        ...
        <Tooltip label="Caste" withArrow>
         <Badge >
            {props.recommend.caste}
         </Badge>
         </Tooltip>
      </Group>

      <Group grow position='center' mb={'sm'}>
         <Button onClick={() => navigate(`/home/main/profile/${props.recommend.id}/about`)} leftIcon={<HiOutlineViewGrid size={20} />} style={{backgroundColor: 'var(--secondary)'}} size='md' variant='filled'>View</Button>
         {props.recommend.isConnected ? (
         <Button size='md' variant='outline'>Connected</Button>
         ): (
            <>
           {props.recommend.isPending ? (

                <Button leftIcon={<VscDebugDisconnect size={20} />}  size='md' variant='outline' >Pending</Button>
              ): (

               <Button leftIcon={<VscDebugDisconnect size={20} />}  size='md' variant='outline'  onClick={() => sendConnectionRequest()}>Connect</Button>
              )}
            </>
         )}
      </Group>
