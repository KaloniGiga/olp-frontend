...
function ForgetPassword() {
   ...
   const dispatch = useDispatch();
    ...
    const handleSubmit = (e) => {
       ...
       .then((res) => [
         dispatch(addToast({kind: 'SUCCESS', msg: 'Email resent successfully.'}))
       ])
       .catch((error) => {
         console.log(error);
  ...
  return (
      <div className='w-screen h-screen flex justify-center items-center bg-screen'>
        ...
        <div className='flex flex-col w-[60%] bg-white px-4 py-4 rounded-xl shadow-md'>
            <h3 className='text-center font-bold my-2 md:text-2xl'>Recover Account</h3>
            ...
            <form action="" onSubmit={(e) => handleSubmit(e)}>
                <Input value={values.email} onChange={(e) => setValues({ email: e.target.value }) } name="email" classes2='font-semibold mt-3' classes="px-2 outline-none border-2 border-rgba(0, 0, 0, 0.4)" label="Email Address" placeholder="Enter your email" type="email" />
