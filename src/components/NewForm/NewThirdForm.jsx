...
import { Button, Group, Paper, Select, TextInput, Title, createStyles } from '@mantine/core';
  ...
  const [user, setUser] = useContext(AuthContext);
     ...
     const handleSubmit = (value) => {
        ...
        // }
         ...
         axiosInstance.post('/users/education-detail', value)
          .then((response) => {
...
const profileOptions = [
 ...
 useEffect(() => {
      const targetCaste = educationQualificationOptions.find((caste) => caste.value == thirdForm.values.education_degree);
      console.log(targetCaste)
      if(!targetCaste && thirdForm.values.education_degree) {
         setEducationQualificationOptions([...educationQualificationOptions, {value: thirdForm.values.education_degree, label: thirdForm.values.education_degree}])
      }
  }, [thirdForm])

   useEffect(() => {
      const targetCaste = subjectOptions.find((caste) => caste.value == thirdForm.values.subject);
      console.log(targetCaste)
      if(!targetCaste && thirdForm.values.subject) {
         setSubjectOptions([...subjectOptions, {value: thirdForm.values.subject, label: thirdForm.values.subject}])
      }
  }, [thirdForm])
  ...
  return (
        ...
        <Paper className={classes.formWrapper}  withBorder radius={2} py={30} px={30}>
         ...
         <Title order={largeDesktop ? 1 : (mediumDesktop ? 3 : 2)} ta="center" mt="md" mb={'md'}>
           ...
           <Group grow mb={largeDesktop ? 'xl' : (mediumDesktop ? 'md' : 'lg')}> 
             <Select
              ...
              value={thirdForm.values.education_degree} onChange={(value) => thirdForm.setFieldValue('education_degree', value)} required searchable creatable label="Educational Qualification" placeholder='Enter highest level of education qualification' size={largeDesktop ? 'xl' : (mediumDesktop ? 'sm' : 'lg')} data={educationQualificationOptions} />
             <Select
              onCreate={(query) => {
                             const item = { value: query, label: query};
                 ...
                 getCreateLabel={(query) => `+Create ${query}`}
              value={thirdForm.values.subject} onChange={(value) => thirdForm.setFieldValue('subject', value)} required searchable creatable label="Field/Subject/Program" placeholder='Enter major subject' size={largeDesktop ? 'xl' : (mediumDesktop ? 'sm' : 'lg')} data={subjectOptions} />
           </Group>
          ...
          <Group grow mb={largeDesktop ? 'xl' : (mediumDesktop ? 'md' : 'lg')}> 
            <TextInput value={thirdForm.values.college} onChange={(e) => thirdForm.setFieldValue('college', e.currentTarget.value)} required label="College/University Name" placeholder="college you attended" size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')}  />
             <TextInput value={thirdForm.values.occupation} onChange={(e) => thirdForm.setFieldValue('occupation', e.currentTarget.value)} required label="Current Profession and Position" placeholder='Enter current profession' size={largeDesktop ? 'xl' : (mediumDesktop ? 'sm' : 'lg')} />
           </Group>
           ...
           <Group grow mb={largeDesktop ? 'xl' : (mediumDesktop ? 'md' : 'lg')}> 
            <Select value={thirdForm.values.sector} onChange={(value) => thirdForm.setFieldValue('sector', value)} required label="Sector" searchable creatable placeholder="Select sector you are working" size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')} data={sectorOptions} />
             <TextInput value={thirdForm.values.companyName} onChange={(e) => thirdForm.setFieldValue('companyName', e.currentTarget.value)} required label="Institution/Company Name" placeholder='Enter name of the employer' size={largeDesktop ? 'xl' : (mediumDesktop ? 'sm' : 'lg')} />
           </Group>
          ...
          <Group grow mb={largeDesktop ? 'xl' : (mediumDesktop ? 'md' : 'lg')}> 
            <Select value={thirdForm.values.annualIncome} onChange={(value) => thirdForm.setFieldValue('annualIncome', value)} required label="Annual Income" placeholder="Select yearly income range" size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')} data={annualIncomeOptions} />
           </Group>
      ...
      <Group position="center" mt="xl">
        <Button className={classes.button} variant="default" onClick={prevStep}>Back</Button>
        <Button type='submit' className={classes.button} variant="outline">Next</Button>
      </Group>
