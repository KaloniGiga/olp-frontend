...
import { Button, Group, NumberInput, Paper, Select, TextInput, Title, createStyles } from '@mantine/core';
...
function NewSecondForm({ secondForm, prevStep, nextStep}) {
  ...
  const handleSubmit = (value) => {
        ...
        // }
         ...
         axiosInstance.post('/users/family-detail', value)
          .then((response) => {
    ...
    <div className="mt-8 xl:mt-12 2xl:mt-16 min-h-full mb-4 px-2 w-[100%] md:w-[100%] lg:w-[95%] xl:w-[80%] rounded-lg mx-auto">
      {/* <h1 className="text-2xl w-full text-center font-semibold xl:text-4xl my-4">Some Personal details</h1> */}
        ...
        <Paper className={classes.formWrapper}  withBorder radius={2} py={30} px={30}>
         ...
         <Title order={largeDesktop ? 1 : (mediumDesktop ? 3 : 2)} ta="center" mt="md" mb={'md'}>Family Information</Title> 
          ...
          <form className="mx-auto" onSubmit={secondForm.onSubmit(handleSubmit)}>
           ...
           <Group grow mb={largeDesktop ? 'xl' : (mediumDesktop ? 'md' : 'lg')}> 
                 <Select value={secondForm.values.familyType} onChange={(value) => secondForm.setFieldValue('familyType', value)} required searchable creatable label="Family Type" placeholder='Select family type' size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')} data={familytypeOptions} />
                 <NumberInput value={Number(secondForm.values.noOfFamilyMember)} onChange={(value) => secondForm.setFieldValue('noOfFamilyMember', value)} required label="No of Family Member" placeholder='Enter no. of family members' size={largeDesktop ? 'xl' : (mediumDesktop ? 'sm' : 'lg')} />
           </Group>
           ...
           <Group grow mb={largeDesktop ? 'xl' : (mediumDesktop ? 'md' : 'lg')}> 
                 <Select value={secondForm.values.liveWithFamily} onChange={(value) => secondForm.setFieldValue('liveWithFamily', value)} required searchable creatable label="Do you live with your family?" placeholder='Select...' size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')} data={liveWithFamilyOptions} />
                 <NumberInput value={Number(secondForm.values.noOfSiblings)} onChange={(value) => secondForm.setFieldValue('noOfSiblings', value)} required label="No of Siblings" placeholder='Enter no. of siblings' size={largeDesktop ? 'xl' : (mediumDesktop ? 'sm' : 'lg')} />
           </Group>
           ...
           <Group grow mb={largeDesktop ? 'xl' : (mediumDesktop ? 'md' : 'lg')}> 
                <TextInput value={secondForm.values.nativePlace} onChange={(e) => secondForm.setFieldValue('nativePlace', e.currentTarget.value)} required label="Native Place" placeholder="Enter Native Place" size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')}  />

                <TextInput value={secondForm.values.motherTongue} onChange={(e) => secondForm.setFieldValue('motherTongue', e.currentTarget.value)} required label="Mother Tongue" placeholder="Enter Mother Tongue" size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')}  />
           </Group>
           ...
           <Group grow mb={largeDesktop ? 'xl' : (mediumDesktop ? 'md' : 'lg')}>
                <TextInput value={secondForm.values.gotra} onChange={(e) => secondForm.setFieldValue('gotra', e.currentTarget.value)} required label="Gotra" placeholder="Enter gotra" size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')}  />
                
                 <Select value={secondForm.values.familyValues} onChange={(value) => secondForm.setFieldValue('familyValues', value)} required searchable creatable label="Family Value" placeholder='Select Family Values' size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')} data={familyValueOptions} />
            </Group> 
             ...
             <Group grow mb={largeDesktop ? 'xl' : (mediumDesktop ? 'md' : 'lg')}>
                <Select value={secondForm.values.parentStatus} onChange={(value) => secondForm.setFieldValue('parentStatus', value)} required searchable creatable label="Parent Status" placeholder='Select Parent Status' size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')} data={parentStatusOptions} />
                 <NumberInput value={Number(secondForm.values.noOfUnmarried)} onChange={(value) => secondForm.setFieldValue('noOfUnmarried', value)} required label="No of Unmarried Sibling " placeholder='Enter no. of Unmarried Siblings' size={largeDesktop ? 'xl' : (mediumDesktop ? 'sm' : 'lg')} />
             </Group>
             ...
             <Group grow mb={largeDesktop ? 'xl' : (mediumDesktop ? 'md' : 'lg')}>
                ...
                <TextInput value={secondForm.values.familyAddress} onChange={(e) => secondForm.setFieldValue('familyAddress', e.currentTarget.value)} required label="Where do your family live?" placeholder="Enter family address" size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')}  />
            </Group>
