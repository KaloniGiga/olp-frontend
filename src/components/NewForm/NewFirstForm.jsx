...
import { Group, Paper, Select, Slider, Text, TextInput, Title, createStyles, rem, Button } from '@mantine/core';
...
useEffect(() => {
  ...
  const [subcasteOptions, setSubcasteOptions] = useState([
    {value: 'Not Applicable', label: 'Not Applicable'},
      ...
      const handleSubmit = (value) => {
        ...
        // }
        console.log(value);
         axiosInstance.post('/users/personal-detail', value)
          .then((response) => {
  ...
  const mediumDesktop = useMediaQuery('(max-width: 1440px)')
   ...
   useEffect(() => {
      const targetCaste = profileOptions.find((profile) => profile.value == firstForm.values.profileCreatedFor);
      
      if(!targetCaste && firstForm.values.profileCreatedFor) {
         setProfileOptions([...profileOptions, {value: firstForm.values.profileCreatedFor, label: firstForm.values.profileCreatedFor}])
      }
  }, [firstForm])

  useEffect(() => {
      const targetCaste = casteOptions.find((caste) => caste.value == firstForm.values.caste);
      console.log(targetCaste)
      if(!targetCaste && firstForm.values.caste) {
         setCasteOptions([...casteOptions, {value: firstForm.values.caste, label: firstForm.values.caste}])
      }
  }, [firstForm])
  
   useEffect(() => {
      const targetReligion = religionOptions.find((religion) => religion.value == firstForm.values.religion);
      if(!targetReligion && firstForm.values.religion) {
         setReligionOptions([...religionOptions, {value: firstForm.values.religion, label: firstForm.values.religion}])
      }
  }, [firstForm])

   useEffect(() => {
      const targetSubCaste = subcasteOptions.find((caste) => caste.value == firstForm.values.subcaste);
      
      if(!targetSubCaste && firstForm.values.subcaste) {
         setSubcasteOptions([...subcasteOptions, {value: firstForm.values.subcaste, label: firstForm.values.subcaste}])
      }
  }, [firstForm])
  ...
  return (
        ...
        <Paper className={classes.formWrapper}  withBorder radius={2} py={30} px={30}>
          {/* <form className="mx-auto" onSubmit={(e) => handleSubmit(e)}> */}
             <Title order={largeDesktop ? 1 : (mediumDesktop ? 3 : 2)} ta="center" mt="md" mb={'md'}>Basic Personal Information</Title> 
          ...
          <form className="mx-auto" onSubmit={firstForm.onSubmit(handleSubmit)}>
             <Group grow mb={largeDesktop ? 'xl' : (mediumDesktop ? 'md' : 'lg')}>
                ...
                required
                 label="Full name"
                  placeholder="Dipak Kalauni"
                  value={firstForm.values.fullname}
                  error={firstForm.errors.fullname && 'Fullname must be more than 3 characters.'}
                  onChange={(e) => firstForm.setFieldValue('fullname', e.currentTarget.value)}
                  size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')}  />
                
                 <Select required searchable creatable 
                 ...
                 onChange={(value) => firstForm.setFieldValue('profileCreatedFor', value)}
                 size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')} data={profileOptions} />
             </Group>
           ...
           <Group grow mb={'sm'}>
             <Select required value={firstForm.values.sex} onChange={(value) => firstForm.setFieldValue('sex', value)} label="Gender" size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')} placeholder='Enter your gender'  data={genderOptions} />
             <Select required value={firstForm.values.religion}
             defaultValue={firstForm.values.religion}
              onChange={(value) => firstForm.setFieldValue('religion', value)} size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')} searchable  creatable
              onCreate={(query) => {
                             const item = { value: query, label: query};
                 ...
                 getCreateLabel={(query) => `+Create ${query}`}
             label="Religion" placeholder='Enter your religion' data={religionOptions} />
           </Group>
           ...
           <Group grow mb={largeDesktop ? 'md' : 'sm'}>
            <Select
             ...
             onCreate={(query) => {
                 ...
                 getCreateLabel={(query) => `+Create ${query}`}
             value={firstForm.values.caste} onChange={(value) => firstForm.setFieldValue('caste', value)} size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')} label="Caste" placeholder='Enter Caste (if applicable)' searchable creatable data={casteOptions}/>
             <Select
              onCreate={(query) => {
                             const item = { value: query, label: query};
                 ...
                 getCreateLabel={(query) => `+Create ${query}`}
              value={firstForm.values.subcaste} onChange={(value) => firstForm.setFieldValue('subcaste', value)} size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')} label="Sub Caste" placeholder='Enter subcaste (if applicable)' searchable creatable data={subcasteOptions} />
           </Group>
            ...
            <Group grow mb={largeDesktop ? 'md' : 'sm'}>
                <DatePickerInput
                 required
                 value={(firstForm.values.month && firstForm.values.day && firstForm.values.year ) ?  new Date(`${firstForm.values.month}/${firstForm.values.day}/${firstForm.values.year}`) : null}
                 onChange={(value) => {
          ...
          <Group grow mb={largeDesktop ? 'md' : 'sm'}>
             <TextInput required value={firstForm.values.address} onChange={(e) => firstForm.setFieldValue('address', e.currentTarget.value)} label="Where do you live?" placeholder='Enter your current address' size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')} />
             <Select required value={firstForm.values.marital_status} onChange={(value) => firstForm.setFieldValue('marital_status', value)} label="Marital Status" placeholder='Select marital status' data={maritalStatusOptions} size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')}/>
          </Group>
             {/* <div className="w-[90%] mx-auto lg:w-full flex flex-col lg:flex-row lg:justify-around  items-center"> 
        ...
        <Group grow mb={largeDesktop ? 'md' : 'sm'}>
            <Select required value={firstForm.values.height} onChange={(value) => firstForm.setFieldValue('height', value)} label={'Height'} placeholder='Select height' data={heightOptions} size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')}
             />

           <Select  required value={firstForm.values.physicalDisability} onChange={(value) => firstForm.setFieldValue('physicalDisability', value)} size={largeDesktop ? 'xl': (mediumDesktop ? 'sm' : 'lg')} label="Any Disability" placeholder='Selct your condition' data={disabilityOptions}/>
            {/* <Slider 
      ...
      <Group position="center" mt="xl">
        ...
        <Button type='submit' className={classes.button} variant="filled" style={{backgroundColor: 'var(--secondary)'}}>Next</Button>
      </Group>
