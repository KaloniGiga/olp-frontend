...
import { addMoreSearchUserThunk, fetchSearchUserThunk } from '../store/thunk/searchUserThunk';
...
function SearchResultSection() {
    ...
    useEffect(() => {
     if(!initial) {
      dispatch(
        addMoreSearchUserThunk({value:name, page, limit})
      )
     }
    }, [page])
  ...
  return (
      <div className=' lg:mx-3 lg:my-3 rounded-xl mt-2 mb-2 lg:mt-4 flex flex-col min-h-[75vh] lg:min-h-[100vh] px-2 py-2 lg:bg-[var(--screen)]'>
    ...
    
      {
        filteredResult.length > 0 ? (
          filteredResult.map((recommend, index) => {
           return (
             <RecommendItem key={index} recommend={recommend} type={'filteredResult'} />
           )
        })
        ): (
            ...
            result.map((recommend, index) => {
               return (
                <NewProfileCard key={index} recommend={recommend} type={'searched'}/>
               )
            })
         ...
         {result && result.length % 20 === 0 && <div className='w-full flex justify-center items-center rounded-3xl'>
          ...
          <div className='w-[100px] h-[100px] md:w-[150px]  md:h-[150px]  relative'>
           ...
           <span className='absolute bottom-0 lg:bottom-5 left-[20%] md:left-[30%] lg:left-[30%] font-semibold  '>No Data</span>
          </div>
        </div>
      )}
    </div>
